...
export function MobileHomePageClient({ initialGuestStatus }: HomePageClientProps) {
    ...
    return (
        <div className="flex flex-col flex-1 overflow-hidden bg-gradient-to-b from-slate-100/90 via-indigo-50/30 to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 relative">
            ...
            <motion.div
                ...
                className="absolute -top-16 -left-16 w-[320px] h-[320px] bg-gradient-to-br from-indigo-400/25 via-purple-400/15 to-pink-400/10 dark:from-indigo-600/30 dark:via-purple-600/20 dark:to-emerald-500/10 rounded-full blur-[80px] pointer-events-none -z-10"
            />
            <motion.div
                animate={{
                    x: [0, -30, 20, 0],
                ...
                className="absolute -bottom-16 -right-16 w-[340px] h-[340px] bg-gradient-to-tr from-emerald-400/20 via-teal-400/15 to-indigo-400/20 dark:from-emerald-600/25 dark:via-teal-600/20 dark:to-indigo-600/25 rounded-full blur-[90px] pointer-events-none -z-10"
            />
            ...
            >
                <motion.div
                    ...
                    className="w-full max-w-[340px] sm:max-w-sm flex flex-col gap-2.5 mx-auto my-auto"
                >
                    <motion.div 
                    ...
                    >
                            <h1 className="text-2xl sm:text-[38px] font-extrabold leading-[1.08] tracking-[-0.03em] bg-clip-text text-transparent bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-600 dark:from-white dark:via-slate-200 dark:to-indigo-400 pb-0.5">
                                Shorten your URL
                            </h1>
                            {!authLoading && (
                                <div className="mt-2 flex flex-wrap items-center justify-center gap-1.5 sm:gap-2">
                                    {user ? (
                                        ...
                                        <Link href="/guest-policy" onClick={(e) => handleLinkClick(e, "/guest-policy")} className="group relative overflow-hidden flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-500/10 dark:bg-amber-500/20 backdrop-blur-md border border-amber-500/30 dark:border-amber-500/40 text-amber-800 dark:text-amber-300 hover:border-amber-400 transition-all duration-300 text-[10px] font-bold tracking-wide cursor-pointer">
                                            ...
                                            <Lock className="w-3 h-3 text-amber-600 dark:text-amber-400 group-hover:text-amber-700 transition-colors relative z-10" />
                                            <span className="relative z-10">1 free link</span>
                                            <span className="text-amber-400/80 relative z-10">•</span>
                                            <Clock className="w-3 h-3 text-amber-600 dark:text-amber-400 group-hover:text-amber-700 transition-colors relative z-10" />
                                            <span className="relative z-10">{initialGuestStatus.guestTtlMs ? Math.round(initialGuestStatus.guestTtlMs / 60000) + 'm' : '5m'}</span>
                                            <ArrowRight className="w-3 h-3 ml-0.5 text-amber-500 dark:text-amber-400 group-hover:translate-x-0.5 transition-all relative z-10" />
                                        </Link>
                                    )}
                                    <button
                                        onClick={togglePageTheme}
                                        title={`Switch to ${pageTheme === "light" ? "Dark" : "Light"} theme`}
                                        className="group relative overflow-hidden flex items-center px-2.5 py-1 rounded-full bg-slate-100/90 dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 text-slate-700 dark:text-slate-200 hover:border-slate-300 dark:hover:border-slate-600 transition-all duration-300 text-[10px] font-bold tracking-wide cursor-pointer shadow-sm"
                                    >
                                        {pageTheme === "light" ? (
                                            <>
                                                <Moon className="w-3 h-3 mr-1 text-indigo-500 transition-transform group-hover:rotate-12" />
                                                <span>Dark</span>
                                            </>
                                        ) : (
                                            <>
                                                <Sun className="w-3 h-3 mr-1 text-amber-400 transition-transform group-hover:rotate-45" />
                                                <span>Light</span>
                                            </>
                                        )}
                                    </button>
                        ...
                        {isStrictlyLoading ? (
                            ...
                            >
                                ...
                                <div className="mt-3 text-center text-[11px] text-muted-foreground/70 leading-relaxed px-4">
                                    By shortening a URL, you agree to our <a href="/terms" target="_blank" className="font-medium text-muted-foreground/80 hover:text-foreground active:text-emerald-600 transition-colors whitespace-nowrap cursor-pointer">Terms of Service <ExternalLink className="inline-block w-[10px] h-[10px] mb-[2px] opacity-60" /></a>, <a href="/acceptable-use" target="_blank" className="font-medium text-muted-foreground/80 hover:text-foreground active:text-emerald-600 transition-colors whitespace-nowrap cursor-pointer">Acceptable Use Policy <ExternalLink className="inline-block w-[10px] h-[10px] mb-[2px] opacity-60" /></a>, and <a href="/code-of-conduct" target="_blank" className="font-medium text-muted-foreground/80 hover:text-foreground active:text-emerald-600 transition-colors whitespace-nowrap cursor-pointer">Code of Conduct <ExternalLink className="inline-block w-[10px] h-[10px] mb-[2px] opacity-60" /></a>.
                                </div>
                            </motion.div>
                        )}
                    </AnimatePresence>
